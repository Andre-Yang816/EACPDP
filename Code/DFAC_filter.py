...
def DFAC_filter(classifier,train_data_x, train_data_y, test_data_x,test_data_y):
    train_data_len = len(train_data_x)
    all_data = np.concatenate((train_data_x, test_data_x), axis=0)
    all_size = len(all_data)
    cluster_num = int(all_size / 20)
    ...
    clusters = AgglomerativeClustering(n_clusters=cluster_num).fit_predict(data_distance)
    final_instance = []
