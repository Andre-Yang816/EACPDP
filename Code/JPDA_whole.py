...
def JPDA_func2(data_count,classify_name,Xs, Ys, Xt, Yt):
    ...
    result = evaluate_classify(Yt,pred)
    return model,result

def result_output_classify(result_list, model_name):
    df = pd.DataFrame(data=result_list, columns=['Accuracy', 'Precision', 'Recall', 'F1_measure','AUC','TN', 'FP', 'FN', 'TP'])
    max = []
    min = []
    median = []
    mean = []
    for i in range(df.shape[1]):
        ...
        tmp_array = np.array(df.iloc[:, i].tolist())
    tmp = [max, median, mean, min]
    df2 = pd.DataFrame(data=tmp,columns=['Accuracy', 'Precision', 'Recall',  'F1_measure','AUC','TN', 'FP', 'FN', 'TP'])
    df2.rename(index={0: 'max',1: 'median', 2: 'mean', 3: 'min'},
               inplace=True)
    performance = pd.concat([df, df2])
    if not os.path.exists("../output/"):
        os.makedirs("../output/")
    performance.to_csv("../output/"  + model_name + '.csv')
    ...
    for cl_name in classifiers_name:
        for root, dirs, files, in os.walk(data_path):
            for file in files:
                ...
                model, result_tmp = JPDA_func2(data_count, cl_name, train_data_x, train_data_y, test_data_x,
                                               test_data_y)
        ...
        model_name = 'JPDA_{0}'.format(cl_name)
        result_output_classify(result, model_name)
