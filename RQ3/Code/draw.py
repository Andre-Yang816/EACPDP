...
def ShowGRAHeatMap(DataFrame, method, func):
    ylabels = DataFrame.columns.values.tolist()
    f, ax = plt.subplots(figsize=(10, 10))
    ax.set_title('{0}'.format(method),fontsize=18,weight='heavy',family='Times New Roman')
    ...
    plt.savefig('../picture/{0}_{1}.png'.format(func,method))
...
if __name__ == '__main__':
    ...
    for func in functions:
        for method in methods:
            data = pd.read_csv("../output_kendell/{0}_{1}.csv".format(func,method))
            ShowGRAHeatMap(data, method, func)
